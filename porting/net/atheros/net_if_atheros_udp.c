 ...
 * @endif
 *
 * @brief Net_dev_operations implementation for the Atheros network stack for UDP
 **/
#include <string.h>
#include <stdlib.h>
#include <assert.h>
...
static int32_t net_if_socket(struct net_dev_operations *self, struct net_socket **new_socket)
{
    ...
    (void)self;
    socketHandle = t_socket((void *) gehandle, ATH_AF_INET, SOCK_DGRAM_TYPE, 0);
...
static int net_if_receive(struct net_socket *socket, char *buf, size_t sz, size_t *nbytes_received)
{
    ...
    received = t_recvfrom((void *) gehandle, socketHandle, (void **) &ptr, sz, 0,
...
static int net_if_send(struct net_socket *socket, const char *buf, size_t sz, size_t *nbytes_sent)
{
    ...
    memset(&hostAddr, 0, sizeof(hostAddr));
    hostAddr.sin_addr = socket->addr;
    hostAddr.sin_port = socket->port;
    hostAddr.sin_family = ATH_AF_INET;
    while ((buffer = CUSTOM_ALLOC(sz)) == NULL)
        ;
    memcpy(buffer, buf, sz);
    error = t_sendto((void *) gehandle, socketHandle,
            (unsigned char *) &buffer[0], sz, 0, (&hostAddr), sizeof(hostAddr));
    if (buffer)
        CUSTOM_FREE(buffer);
