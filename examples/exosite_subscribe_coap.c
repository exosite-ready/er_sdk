 ...
 *  @endif
 *
 *  @example exosite_subscribe_coap.c
 *
 *  This example demonstrates how to subscribe to a data resource on the
 *  server over HTTP.
 *  It subscribes to the "Dev1" data resource, show a message when its
 *  value is updated. (For example a new value is set on the OpenTether
 *  portal)
 *
 *  @note
...
int main(int argc, char *argv[])
{
    ...
    info_log(DEBUG_APPLICATION, ("Coap subscribe example\n"));

    /** Create new instance */
    error = exosite_new(&exo, vendor, model, sn, APP_PROTO_COAP);
    if (error)
        return ERR_FAILURE;
